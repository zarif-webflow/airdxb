...
(() => {
  ...
  var form = assertValue(document.querySelector("form[data-calculator-form]"));
  var step1Element = assertValue(form.querySelector('[data-form-step="1"]'));
  var nextButton = assertValue(
    step1Element.querySelector("button[data-next-button]")
  );
  var step2Element = assertValue(form.querySelector('[data-form-step="2"]'));
  var prevButton = assertValue(
    step2Element.querySelector("button[data-prev-button]")
  );
  var step1InputElements = [
    assertValue(form.querySelector("[data-address-input]")),
    assertValue(form.querySelector("[data-bedrooms-input]"))
  ];
  var animateNext = async () => {
    await animate2(step1Element, { opacity: 0, transform: "translateX(-100%)" }, { duration: 0.4 }).finished;
    step1Element.classList.add("is--hidden");
    await animate2(step2Element, { opacity: 0, transform: "translateX(100%)" }, { duration: 0 }).finished;
    step2Element.classList.remove("is--hidden");
    await animate2(step2Element, { opacity: 1, transform: "translateX(0%)" }, { duration: 0.4 }).finished;
  };
  var animatePrev = async () => {
    await animate2(step2Element, { opacity: 0, transform: "translateX(100%)" }, { duration: 0.4 }).finished;
    step2Element.classList.add("is--hidden");
    await animate2(step1Element, { opacity: 0, transform: "translateX(-100%)" }, { duration: 0 }).finished;
    step1Element.classList.remove("is--hidden");
    await animate2(step1Element, { opacity: 1, transform: "translateX(0%)" }, { duration: 0.4 }).finished;
  };
